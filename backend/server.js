...
//or ATH->BER->HEL with return HEL->BUD->ATH then the arrival airport is HEL
app.post('/flynow/car-booking-exists', async (req, res) => {
	...
	try {
		...
		if(result.rows.length==0) {
			res.json([{success: false, successAirport: true}])
		}
		else {
			const numOfFlights = await pool.query(`
			SELECT COUNT(DISTINCT flightid) 
			...
			if(numOfFlights.rows[0].count === '1') {
				...
				if(result.rows.length == 0) {
					check = 1
					res.json([{success: true, successAirport: false}])
				}
			}
			else if(numOfFlights.rows[0].count === '2') {
				const result = await pool.query(`SELECT DISTINCT f1.flightid FROM flight f1, flight f2, has h1, has h2 
				WHERE f1.flightid=h1.flightid AND f2.flightid=h2.flightid
				AND h1.reservationid='${jsonArray[0].bookingId.toUpperCase()}'
				AND h2.reservationid='${jsonArray[0].bookingId.toUpperCase()}'
				AND f1.arrivalairport=f2.departureairport AND (
				(f2.arrivalairport=f1.departureairport AND f1.flightdate<=f2.flightdate AND f1.arrivalairport='${jsonArray[0].location}')
				OR (f2.arrivalairport<>f1.departureairport 
				...
				const result = await pool.query(`SELECT DISTINCT f1.flightid FROM flight f1, flight f2, flight f3, 
				has h1, has h2, has h3
				WHERE f1.flightid=h1.flightid AND f2.flightid=h2.flightid 
				AND f3.flightid=h3.flightid
				AND h1.reservationid='${jsonArray[0].bookingId.toUpperCase()}'
				...
				OR (f1.flightdate>=f2.flightdate AND f1.departureairport='${jsonArray[0].location}'))`)
				if(result.rows.length == 0) {
					check = 1
					res.json([{success: true, successAirport: false}])
				}
			...
			if(check === 0) {
				res.json([{success: true, successAirport: true}])
			}
		}
    } catch (error) {
		console.error(error)
